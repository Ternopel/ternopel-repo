function compress() {
	...
	s3cmd -m $mime --add-header=content-type:$mime sync /tmp/public/* -P --verbose --add-header=cache-control:max-age=16070400,public --add-header=content-encoding:gzip s3://ternopel
	echo "Done !!"
